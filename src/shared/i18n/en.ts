import type { MessageKey, LocaleContent } from '@/shared/i18n/types'

export const enMessages: Record<MessageKey, string> = {
  'administrative.province': 'Province/City',
  'administrative.commune': 'Ward/Commune',
  'administrative.selectProvince': 'Choose province/city',
  'administrative.wholeProvince': 'Entire province/city',
  'administrative.clear': 'Clear area',
  'administrative.search': 'Search by name',
  'administrative.empty': 'No matching administrative units',
  'administrative.changed': 'Administrative data changed. Please reselect your province and ward/commune.',
  'common.continue': 'Continue',
  'common.back': 'Back',
  'common.cancel': 'Cancel',
  'common.details': 'Details',
  'common.directions': 'Directions',
  'common.copy': 'Copy',
  'common.copied': '✓ Copied',
  'common.optional': 'Optional',
  'common.retry': 'Try again',
  'common.loading': 'Loading…',
  'common.errorTitle': 'Something went wrong',
  'common.errorBody': 'We could not load this. Check your connection and try again.',
  'common.offlineTitle': 'No connection',
  'common.offlineBody': 'You are offline. Some details may be out of date.',
  'common.permissionDenied': 'You do not have permission to do that.',

  'auth.signInTitle': 'Sign in\nto create a room',
  'auth.signInBody': 'Only the host needs an account. Friends join by link, no sign-up.',
  'auth.signUpTitle': 'Create your\nGoGo account',
  'auth.signUpBody': 'So your rooms, plans and saved places stick around.',
  'auth.signInTab': 'Sign in',
  'auth.signUpTab': 'Sign up',
  'auth.displayName': 'Display name',
  'auth.displayNamePlaceholder': 'What friends will see',
  'auth.displayNameInvalid': 'Use between 1 and 50 characters.',
  'auth.email': 'Email',
  'auth.emailInvalid': 'That email does not look right.',
  'auth.password': 'Password',
  'auth.passwordHint': 'At least 10 characters.',
  'auth.passwordInvalid': 'Password needs at least 10 characters.',
  'auth.signInCta': 'Sign in',
  'auth.signUpCta': 'Create account',
  'auth.invalidCredentials': 'Email or password is incorrect.',
  'auth.registerConflict': 'Could not complete sign-up. Try signing in instead.',
  'auth.rateLimited': 'Too many attempts. Wait a moment and try again.',
  'auth.networkError': 'Connection lost. Check your network and try again.',
  'auth.timeoutError': 'The server took too long to answer. Check your network and try again.',
  'auth.unexpectedError': 'The app hit an unexpected error. Try again, and reopen the app if it keeps happening.',
  'auth.genericError': 'Something went wrong. Please try again.',
  'auth.guestFootnote': 'Joining a room by invite link needs no account.',

  'account.legacyArea': 'This legacy area has no canonical mapping. Select a province and ward/commune to update it.',
  'account.title': 'Account',
  'account.signInBody': 'Sign in to manage your profile and data.',
  'account.save': 'Save',
  'account.saving': 'Saving…',
  'account.saved': '✓ Saved',
  'account.saveFailed': 'Could not save. Please try again.',
  'account.dataTitle': 'Your data',
  'account.exportBody': 'Download everything GoGo stores about you. Up to 3 times an hour.',
  'account.export': 'Export my data',
  'account.exporting': 'Preparing…',
  'account.exportFailed': 'Could not export. Try again later.',
  'account.deleteTitle': 'Delete account',
  'account.deleteBody':
    'Your account can no longer be signed in to, and GoGo removes your personal data: name, avatar, home area, budget, interests, saved items and notifications. This cannot be undone.',
  'account.deleteKept':
    'Reviews and photos you contributed to places stay on GoGo, no longer showing your name.',
  'account.deleteConfirm': 'Delete account',
  'account.deleteCancel': 'Cancel',
  'account.deleting': 'Deleting…',
  'account.deleteFailed': 'Could not delete. Please try again.',
  'account.avatarTitle': 'Profile picture',
  'account.avatarBody': 'People in your rooms will see this picture.',
  'account.avatarUnavailable': 'Picture upload is not available in this environment.',
  'account.avatarChange': 'Change picture',
  'account.avatarRemove': 'Remove',
  'account.avatarUploading': 'Uploading…',
  'account.avatarSaved': '✓ Picture updated',
  'account.avatarRemoved': '✓ Picture removed',
  'account.avatarUnreadable': 'That image could not be read. Pick another one.',
  'account.avatarBusy': 'The server is busy. Try again in a few minutes.',
  'account.avatarUnsupportedType': 'That image format is not supported. Pick a JPG or PNG.',
  'account.avatarPermissionDenied': 'No access to your photos. Allow it in Settings and try again.',
  'account.avatarFailed': 'Could not update the picture. Please try again.',
  'account.defaultsTitle': 'Defaults for a new date',
  'account.defaultsBody': 'GoGo pre-fills a new date from here. Each date can still be changed on its own.',
  'account.homeArea': 'Home area',
  'account.homeAreaNone': 'Not set',
  'account.homeAreaPick': 'Choose an area',
  'account.homeAreaClear': 'Clear',
  'account.areaPickerTitle': 'Choose a home area',
  'account.areaPickerEmpty': 'No areas to choose from yet.',
  'account.areaPickerFailed': 'Could not load the area list.',
  'account.interests': 'Interests',
  'account.interestsHint': 'Up to {{max}}. Offered when you pick preferences in a room.',
  'account.interestsFailed': 'Could not load the interests list.',
  'account.usualBudget': 'Usual budget · per person',
  'account.usualBudgetNone': 'Not set',
  'account.defaultsSave': 'Save defaults',
  'account.defaultsSaved': '✓ Defaults saved',
  'account.dobTitle': 'Date of birth',
  'account.dobBody': 'Optional. Only you can see it; other members never do.',
  'account.dobUnset': 'No date of birth set',
  'account.dobCurrent': 'Saved: {{date}}',
  'account.dobInputLabel': 'Date of birth (day/month/year)',
  'account.dobFormat': 'Enter day/month/year, for example 17/05/1990.',
  'account.dobPlaceholder': '17/05/1990',
  'account.dobInvalid': 'Not a valid date. Enter a real date as day/month/year.',
  'account.dobFuture': 'Date of birth cannot be after today.',
  'account.dobSave': 'Save date of birth',
  'account.dobClear': 'Clear date of birth',
  'account.dobSaved': '✓ Date of birth saved',
  'account.dobCleared': '✓ Date of birth cleared',
  'account.dobFailed': 'Could not save your date of birth. Try again.',
  'account.defaultsFailed': 'Could not save defaults. Please try again.',

  'tabs.home': 'Home',
  'tabs.plans': 'Plans',
  'tabs.saved': 'Saved',
  'tabs.profile': 'Profile',

  'onboarding.skip': 'Skip',
  'onboarding.start': 'Get started',
  'onboarding.next': 'Next',
  'onboarding.guest': 'Continue as guest',
  'onboarding.slide1.title': 'Stop wasting half an hour\non "where to?"',
  'onboarding.slide1.body': 'GoGo finds places and activities that fit all of you.',
  'onboarding.slide2.title': 'Each of you\npicks separately',
  'onboarding.slide2.body': 'No debating. GoGo only shows the options everyone likes.',
  'onboarding.slide3.title': 'Get a complete\ndate plan instantly',
  'onboarding.slide3.body': 'What to eat, where to go, how long it takes and roughly how much.',

  'home.greeting': 'Good evening 👋',
  'home.subtitle': 'Where to today?',
  'home.heroBadge': 'Tonight',
  'home.heroTitle': 'Where to tonight?',
  'home.heroBody': 'GoGo picks places based on your mood, time and budget.',
  'home.createDate': 'Create a plan',
  'home.quickPick': '✨ Quick pick',
  'home.suggested': 'You might like these',
  'home.suggestedHint': 'Picked for you',
  'home.empty': 'No suggestions for this area yet.',
  'home.emptyHint': 'Try a wider radius or a different time.',
  'home.error': "Couldn't load suggestions.",
  'home.retry': 'Retry',
  'home.subtitleHostActive': '{{done}}/{{total}} people have finished picking',
  'home.subtitleGuestActive': '{{n}} picks left in {{name}}’s plan',
  'home.emptyTitle': 'No good plans found in this area',
  'home.emptyBody': 'Loosen one condition so GoGo can find more options.',
  'home.recoverRadius': 'Widen radius to 10 km',
  'home.recoverTime': 'Change the time window',
  'home.recoverBudget': 'Raise the budget',
  'home.recoverNearest': 'Show the closest matches',
  'home.errorBody': 'Check your connection and try again.',
  'home.createManual': 'Build a plan manually',
  'home.scopeGps': 'Near your current location',
  'home.scopeArea': 'In your account area: {{area}}',
  'home.scopeNoneTitle': 'General suggestions, not based on location',
  'home.scopeNoneBody': 'Turn on location or choose an area in your account for suggestions near you.',
  'home.scopeReselectBody': 'Your account area needs choosing again because the administrative data changed.',
  'home.useLocation': 'Use current location',
  'home.chooseArea': 'Choose an area',
  'home.locationDenied': 'Location permission is off. You can still choose an area in your account.',
  'home.locationUnavailable': 'Could not get your location. You can still choose an area in your account.',

  'createLocation.title': 'Where do\nwe start?',
  'createLocation.body': 'GoGo will find places near you that are still open.',
  'createType.roomName': 'Plan name (optional)',
  'createType.roomNameHint': 'e.g. Weekend coffee',
  'draft.reviewSelections': 'Some options have changed. Please select them again before creating the plan.',
  'draft.close': 'Close',
  'draft.exitTitle': 'Leave plan creation?',
  'draft.exitBody': 'Save a draft to continue later.',
  'draft.keepEditing': 'Keep editing',
  'draft.discard': 'Discard draft',
  'draft.saveExit': 'Save and leave',
  'draft.save': 'Save draft',
  'draft.resume': 'Resume draft',
  'draft.delete': 'Delete draft',
  'draft.signIn': 'Sign in to save a draft to your account.',
  'draft.saveFailed': 'Could not save the draft. Please try again.',
  'createType.title': 'Who is this\nplan for?',
  'createType.couple': 'Two people',
  'createType.coupleDesc': 'Pick separately — GoGo finds what you both like.',
  'createType.group': 'Friend group',
  'createType.groupDesc': 'Everyone votes, the host locks it in.',
  'groupSetup.title': 'About how many\npeople are in?',
  'groupSetup.people': '{{n}} people',
  'groupSetup.budgetBy': 'Budget counted',
  'groupSetup.decrease': 'One fewer person',
  'groupSetup.increase': 'One more person',
  'groupSetup.perPersonHelp': 'Budget is per person. Group total = budget × {{n}} people.',
  'groupSetup.groupTotalHelp': 'Budget covers the whole group of {{n}}; divided, that is the per-person share.',
  'groupSetup.perPerson': 'Per person',
  'groupSetup.groupTotal': 'Group total',
  'createLocation.current': 'Current location',
  'createLocation.anywhere': 'Anywhere',
  'createLocation.maxDistance': 'Max distance',
  'createLocation.areaTitle': 'Or choose an area',
  'createLocation.useMyLocation': 'Tap to use your location',
  'createLocation.locating': 'Getting your location…',
  'createLocation.locatedNoLabel': 'Got your location',
  'createLocation.permissionDenied': 'No location permission. Pick an area below instead.',
  'createLocation.locationUnavailable': "Couldn't get your location. Pick an area below instead.",
  'createLocation.useHomeArea': 'Use my home area: {{area}}',

  'createTime.title': 'How much time\ndo you all have?',
  'createTime.title_couple': 'How much time\ndo you two have?',
  'createTime.body': 'GoGo only suggests places still open within this window.',
  'createTime.specificTime': 'Start and end time',
  'createTime.start': 'Start',
  'createTime.pickTime': 'Pick a time',
  'createTime.notSet': '--:--',
  'createTime.startRequired': 'Pick a start time to continue.',
  'createTime.end': 'End',
  'createTime.durationLabel': 'Expected length (optional)',
  'createTime.preset.upTo2h': '1–2 hrs',
  'createTime.preset.upTo3h': '2–3 hrs',
  'createTime.preset.upTo4h': '3–4 hrs',
  'createTime.preset.evening': 'All evening',
  'createTime.presetNeedsStart': 'Pick a start time — GoGo works out the end from this length.',
  'createTime.openEnd': 'Open',
  'createTime.nextDay': '(next day)',

  'createBudget.title': 'Budget\nfor both?',
  'createBudget.body': 'Estimated total for two people.',
  'createBudget.tier.under300': 'Under 300k',
  'createBudget.tier.under300.sub': 'Coffee · a walk',
  'createBudget.tier.to500': '300–500k',
  'createBudget.tier.to500.sub': 'Light bite · bubble tea',
  'createBudget.tier.to800': '500–800k',
  'createBudget.tier.to800.sub': 'Dinner · a movie',
  'createBudget.tier.to1500': '800k–1.5m',
  'createBudget.tier.to1500.sub': 'Restaurant · workshop',
  'createBudget.tier.flexible': "Doesn't matter",
  'createBudget.tier.flexible.sub': 'As long as it is fun',
  'createBudget.useUsual': 'Use my usual budget: {{tier}}',

  'createMood.title': "Tonight's\nmood?",
  'createMood.body': 'Pick up to {{max}} moods.',
  'createMood.settingTitle': 'Setting',
  'createMood.spendingTitle': 'Spending style',
  'createMood.seedTitle': 'Places you want to suggest',
  'createMood.seedHint': 'Optional — GoGo will prioritize them in the plan.',
  'createMood.addPlace': '＋ Add place',
  'createMood.removePlace': 'Remove {{name}} from the list',
  'createMood.cta': 'Create GoGo Room',
  'createMood.creating': 'Creating room…',
  'createMood.createFailed': 'Could not create the room. Please try again.',

  'gogoRoom.createInvite': 'Create invite code',
  'gogoRoom.inviteFailed': 'Could not create an invite code. You can still manage the room and choose preferences.',
  'gogoRoom.inviteActive': 'This room has an invite code valid until {{time}}. The code only shows on the device that created it.',
  'gogoRoom.reissueInvite': 'Create a new code',
  'gogoRoom.reissueTitle': 'Create a new invite code?',
  'gogoRoom.reissueBody': 'The current code will stop working. People already in the room are not affected; anyone who has not joined needs the new code.',
  'gogoRoom.reissueBodyMany': 'All {{n}} codes in use will stop working. People already in the room are not affected; anyone who has not joined needs the new code.',
  'gogoRoom.inviteClosed': 'This room no longer takes new members, so its invite codes cannot be used to join.',
  'gogoRoom.reissueConfirm': 'Revoke and create new',
  'gogoRoom.reissueCancel': 'Not now',
  'gogoRoom.reissueFailed': 'Could not revoke the current code. Try again.',
  'gogoRoom.invitesLoadFailed': "Could not check this room's invite codes.",
  'gogoRoom.partialTitle': 'Continue with current responses?',
  'gogoRoom.partialBody': '{{n}} members have not finished. They remain in the room and can vote; party size and budget stay unchanged.',
  'gogoRoom.partialContinue': 'Continue with current responses',
  'gogoRoom.quorumRequired': 'At least 2 members must finish their preferences before matching.',
  'gogoRoom.title': 'Now pull the other one in',
  'gogoRoom.title_group': 'Now pull the crew in',
  'gogoRoom.body': "You'll each pick separately. Nobody sees the other's choices until you match.",
  'gogoRoom.body_group': "Everyone picks separately. Nobody sees each other's choices until the results are in.",
  'gogoRoom.codeLabel': 'GoGo Room code',
  'gogoRoom.constraints': 'Room settings',
  'gogoRoom.membersTitle': 'Members · {{joined}}/{{total}} finished picking',
  'gogoRoom.hostBadge': 'Host',
  'gogoRoom.guestBadge': 'Guest',
  'gogoRoom.status.waiting': 'Not started',
  'gogoRoom.status.choosing': 'Picking',
  'gogoRoom.status.completed': 'Done',
  'gogoRoom.invite': 'Invite people 📩',
  'gogoRoom.linkCopied': '✓ Invite link copied',
  'gogoRoom.noApp': "The other person doesn't need the app.",
  'gogoRoom.noApp_group': "Nobody needs the app.",
  'gogoRoom.previewGuest': "Preview the guest's screen",
  'gogoRoom.joined': '{{joined}}/{{total}} joined',
  'gogoRoom.myPreferences': 'Pick your preferences',
  'gogoRoom.returnToVote': 'Return to voting',
  'gogoRoom.returnToResult': 'View voting results',
  'gogoRoom.startMatching': 'Start matching ✨',
  'gogoRoom.starting': 'Matching…',
  'gogoRoom.startFailed': 'Could not start matching. Please try again.',
  'gogoRoom.planUnavailable': "Couldn't open this room's plan, so here is the room.",
  'gogoRoom.planRetry': "Couldn't load this room's plan. Check your connection and try again.",
  'roomManage.title': 'Manage room',
  'roomTitle.title': 'Plan name',
  'roomTitle.body': 'Leave it empty to remove the name. Everyone in the room sees the new name.',
  'roomTitle.saved': '✓ Plan renamed',
  'roomTitle.cleared': '✓ Name removed',
  'roomTitle.locked': 'This plan can no longer be renamed.',
  'roomTitle.tooLong': 'A plan name can be at most 80 characters.',
  'roomManage.hostOnlyTitle': 'Host only',
  'roomManage.hostOnlyBody': 'Only whoever created the room can change this.',
  'roomManage.budgetTitle': 'Budget',
  'roomManage.staleWarning': '⚠️ Changing a constraint makes the current suggestions and plan out of date — they need rebuilding.',
  'roomManage.savedStale': '✓ Saved · suggestions and plan need rebuilding',
  'roomManage.saveFailed': 'Could not save. Please try again.',
  'roomManage.conflict': 'Someone else just changed the constraints. Reload and try again.',
  'roomManage.membersTitle': 'Members',
  'roomManage.noMembers': 'Nobody has joined yet.',
  'roomManage.role.host': 'Host',
  'roomManage.role.member': 'Member',
  'roomManage.selection.pending': 'Not started',
  'roomManage.selection.in_progress': 'Picking',
  'roomManage.selection.completed': 'Done',
  'roomManage.removeTitle': 'Remove {{name}} from the room?',
  'roomManage.removeBody': 'They lose access to this room.',
  'roomManage.removeConfirm': 'Remove',
  'roomManage.invitesTitle': 'Invites',
  'roomManage.invitesBody': 'Invite codes are shown once when created and cannot be listed again.',
  'roomManage.invite': 'Invite',
  'roomManage.inviteUses': 'Invite · up to {{n}} uses',
  'roomManage.revoke': 'Revoke',
  'roomManage.addSeedPlaces': 'Add suggested places',
  'roomManage.cancelRoom': 'Cancel room',
  'roomManage.cancelling': 'Cancelling…',
  'roomManage.cancelTitle': 'Cancel this room?',
  'roomManage.cancelBody': 'A cancelled room cannot be restored. Everyone loses access.',
  'roomManage.seedTitle': 'Suggested places',

  'guestJoin.badge': 'Date invite',
  'guestJoin.title': "You're invited\nto pick a date 🎉",
  'guestJoin.detailsTitle': 'Date details',
  'guestJoin.join': 'Join 🙌',
  'guestJoin.noAccount': 'No account needed.',
  'guestJoin.asAccount': "You'll join with the account you're signed in with.",
  'guestJoin.you': 'You',
  'guestJoin.nameLabel': 'Display name',
  'guestJoin.namePlaceholder': 'Enter your name',
  'guestJoin.nameRequired': 'Add a name so everyone knows who you are.',
  'guestJoin.joining': 'Joining…',
  'guestJoin.expiredTitle': 'Invite expired',
  'guestJoin.expiredBody': 'This invite no longer works. Ask the host for a new link.',
  'guestJoin.roomNotJoinableBody': "This room isn't taking new people any more, so a new link won't help. Ask the host if you need to join.",
  'guestJoin.roomExpiredBody': "This room has expired, so a new link won't help. Ask the host to start a new room.",
  'guestJoin.notFoundTitle': 'Room not found',
  'guestJoin.notFoundBody': 'The invite link is wrong or the room was deleted.',
  'guestJoin.failed': 'Could not join. Please try again.',
  'shareLink.resolving': 'Opening the link…',
  'shareLink.goneTitle': 'This link no longer works',
  'shareLink.goneBody': 'It expired or was revoked. Ask whoever sent it for a new one.',
  'shareLink.failedTitle': 'Could not open the link',
  'shareLink.failedBody': 'Check your connection and try again.',
  'shareLink.unsupportedTitle': 'This version cannot open that link',
  'shareLink.unsupportedBody': 'Update GoGo, then open the link again.',
  'shareLink.retry': 'Try again',
  'shareLink.goHome': 'Go to home',
  'notFound.title': 'Page not found',
  'notFound.body': 'This address is not part of GoGo, or it has changed.',
  'joinByCode.title': 'Join with a code',
  'joinByCode.body': 'Enter the invite code you were given. With a link, just open it instead.',
  'joinByCode.codeLabel': 'Invite code',
  'joinByCode.codePlaceholder': 'Paste the code here',
  'joinByCode.expired': 'That invite expired or was revoked. Ask the host for a new one.',
  'joinByCode.roomNotJoinable': "This room isn't taking new people any more, so a new code won't help. Ask the host if you need to join.",
  'joinByCode.roomExpired': "This room has expired, so a new code won't help. Ask the host to start a new room.",
  'joinByCode.notFound': 'No room matches that code.',
  'joinByCode.failed': 'Could not join. Please try again.',

  'preference.title': 'What are you\nmore in the mood for?',
  'preference.body': 'Pick the 2–3 things that fit your evening best.',
  'preference.saveFailed': 'Could not save your picks. Please try again.',
  'preference.conflict': 'Your picks changed on another device. Reload and choose again.',
  'preference.useSaved': 'Use my saved interests',

  'suggestion.reason.MATCHES_PREFERENCES': 'Matches your picks',
  'suggestion.reason.LIKED_BY_EVERYONE': 'Everyone likes it',
  'suggestion.reason.FITS_BUDGET': 'Fits the budget',
  'suggestion.reason.NEAR_ORIGIN': 'Near your start',
  'suggestion.reason.HIGHLY_RATED': 'Highly rated',
  'suggestion.reason.HOST_SUGGESTED': 'Suggested by the host',

  'swipe.header': 'Pick what you like',
  'swipe.subheader': 'GoGo will find the overlap',
  'swipe.like': 'LIKE ❤️',
  'swipe.pass': 'PASS ✕',
  'swipe.star': 'REALLY WANT ⭐',
  'swipe.likeLabel': 'Like',
  'swipe.passLabel': 'Not for me',
  'swipe.starLabel': 'Really want',
  'swipe.notReadyTitle': 'No suggestions yet',
  'swipe.notReadyBody': 'Once everyone has picked, the host starts matching.',
  'swipe.goToLobby': 'Back to the room',
  'swipe.doneTitle': "You've voted on everything",
  'swipe.doneBody': 'Waiting on the others to finish voting.',
  'suggestionRun.emptyTitle': 'No suitable places found',
  'suggestionRun.emptyBodyHost': "No place fits this room's conditions yet. Try adjusting the room conditions, then refresh the suggestions.",
  'suggestionRun.emptyBodyMember': "No place fits this room's conditions yet. Wait for the host to adjust them or refresh the suggestions.",
  'suggestionRun.staleTitle': 'Suggestions are out of date',
  'suggestionRun.adjustConstraints': 'Adjust room conditions',

  'matching.matched': "It's a match!",
  'matching.matchedBody': 'You have more in common than you thought.',
  'matching.matchedBody_couple': 'You two have more in common than you thought.',
  'matching.matchedBody_group': 'The group has more in common than you thought.',
  'matching.failedTitle': 'Could not match',
  'matching.notReadyBody': 'The room is not ready yet. Wait for everyone to finish picking.',
  'matching.waitingForHost': 'Waiting for the host to start matching…',
  'matching.reason': 'Filtering by the room\'s budget, opening hours and distance.',
  'matching.retrying': 'Retrying…',

  'swipe.saveFailed': 'Your vote was not saved. Please retry this card.',
  'swipe.votingClosed': 'This room has chosen its plan. Voting is closed.',
  'matchResult.waitingMatch': 'No mutual choice yet. You can review and change your votes.',
  'matchResult.vote': 'Review and vote',
  'matchResult.noVotes': 'No votes yet. Vote before finalizing.',
  'matchResult.roomNotMatching': 'The room state changed. Return to the room for the latest status.',
  'matchResult.whyTitle': 'Why did GoGo pick this?',
  'matchResult.matchBadge': 'GREAT FIT',
  'matchResult.viewPlan': 'View the plan',
  'matchResult.another': 'Show another option',
  'matchResult.refineTitle': 'How should we change the plan?',
  'matchResult.groupTitle': 'The group favorite',
  'matchResult.groupVotes': '{{likes}}/{{total}} liked · {{vetoes}} vetoes',
  'matchResult.customRequest': 'Something else…',
  'matchResult.regenerate': 'Generate new plan',
  'matchResult.suggestToHost': 'Suggest to host',
  'matchResult.suggestTitle': 'What would you suggest?',
  'matchResult.sendSuggestion': 'Send suggestion',
  'matchResult.suggestionSent': '✓ Suggestion sent to the host',
  'matchResult.lockConflict': 'You are keeping {{n}} stops',
  'matchResult.keepLocked': 'Keep these stops',
  'matchResult.dropLocked': 'Drop them and change everything',
  'matchResult.charCount': '{{n}}/150',

  'datePlan.title': 'Plan',
  'datePlan.title_couple': 'Date · {{when}}',
  'datePlan.title_group': 'Group of {{n}} · {{when}}',
  'datePlan.titleUndated': 'Plan',
  'datePlan.titleUndated_couple': 'Date plan',
  'datePlan.titleUndated_group': 'Plan for a group of {{n}}',
  'datePlan.when.today': 'today {{time}}',
  'datePlan.when.tomorrow': 'tomorrow {{time}}',
  'datePlan.when.other': '{{month}}/{{day}} {{time}}',
  'datePlan.when.otherYear': '{{month}}/{{day}}/{{year}} {{time}}',
  'datePlan.match': 'Great fit',
  'datePlan.optionalExtra': 'Add this stop: +{{amount}}',
  'datePlan.for2': 'for 2 people',
  'datePlan.perPerson': '/person',
  'datePlan.groupOf': 'group of {{n}}',
  'datePlan.lockHint': 'Keep this stop when regenerating',
  'datePlan.unlockHint': 'Stop keeping this stop',
  'datePlan.lockedToast': 'Keeping {{name}} when regenerating',
  'datePlan.unlockedToast': 'No longer keeping {{name}}',
  'price.groupTotal': 'group total',
  'price.groupTotalOf': 'group total for {{n}}',
  'price.atLeast': 'from {{amount}}',
  'price.forPeople': 'for {{n}} people',
  'price.unit.per_person': '/person',
  'price.unit.per_group': ' · group total',
  'price.unit.per_item': '/item',
  'price.unit.per_hour': '/hour',
  'price.unit.per_night': '/night',
  'price.unit.free': 'Free',
  'price.unit.unknown': 'No price information yet',

  'rating.google': 'Google',
  'rating.gogo': 'GoGo',
  'rating.gogoInsufficient': 'Not enough reviews from the GoGo community yet',
  'common.open': 'Open now',
  'common.closed': 'Closed',
  'common.staleOffline': 'Offline — this is the copy saved on your device.',
  'common.staleError': "Couldn't refresh — this is the copy saved on your device.",
  'matchResult.diffKept': 'Kept {{name}}',
  'matchResult.diffChanged': 'Swapped {{from}} → {{to}}',
  'matchResult.diffSaved': 'Saves about {{amount}}',
  'matchResult.finalize': 'Lock this in',
  'matchResult.finalizing': 'Locking in…',
  'matchResult.finalizeFailed': 'Could not lock it in. Please try again.',
  'matchResult.waitingHost': 'Waiting for the host to lock it in.',
  'matchResult.regenerating': 'Regenerating…',
  'matchResult.runnersUp': 'Other options',
  'matchResult.points': '{{n}} points',
  'matchResult.noReasons': 'No specific reasons recorded for this pick.',
  'matchResult.emptyTitle': 'No options yet',
  'matchResult.emptyBody': 'Everyone needs to finish picking, then the host starts matching.',
  'matchResult.stale': 'The room constraints just changed — regenerate for an accurate result.',
  'matchResult.staleCause': 'Preferences or room conditions changed, so these suggestions are out of date.',
  'matchResult.refreshStale': 'Refresh suggestions',
  'matchResult.staleWaiting': 'Waiting for the host to refresh the suggestions.',
  'matchResult.regenerateRace': 'Someone changed their answers while suggestions were being built. Try again.',
  'matchResult.regenerateFailed': 'Could not refresh the suggestions. Try again.',
  'datePlan.travelLeg': '~{{n}} min travel',
  'datePlan.locked': 'Locked',
  'datePlan.lockable': 'Lock',
  'datePlan.total': 'Total',
  'datePlan.time': 'Time',
  'datePlan.travel': 'Travel',
  'datePlan.go': "Let's go →",
  'datePlan.stale': 'The room constraints just changed — this plan may be out of date.',
  'datePlan.done': 'Done',
  'datePlan.minutes': '{{n}} min',
  'datePlan.overBudget': '⚠️ Over the budget you set',
  'datePlan.stopOrder': 'Stop {{n}}',
  'datePlan.regenerate': 'Rebuild the plan',
  'datePlan.regenerating': 'Rebuilding…',
  'datePlan.regenerated': '✓ New plan ready',
  'datePlan.regeneratedKept': '✓ New plan · {{n}} locked stops kept',
  'datePlan.regenerateFailed': 'Could not rebuild. Please try again.',
  // #251 — starting the date is the host moving the room to `active`.
  'datePlan.starting': 'Starting…',
  'datePlan.enter': 'Open the outing →',
  'datePlan.waitingHost': 'Waiting for the host to start the outing… This screen updates once it begins.',
  'datePlan.notStartable': 'The room is not ready yet, so the outing cannot start.',
  'datePlan.roomUnavailable': 'Could not load the room status.',
  'datePlan.startOffline': 'Could not start. Check your connection and try again.',
  'datePlan.startHostOnly': 'Only the host can start the outing.',
  'datePlan.startFailed': 'Could not start. Please try again.',
  'datePlan.roomCancelled': 'This room was cancelled, so it can no longer start.',
  'datePlan.roomExpired': 'This room has expired, so it can no longer start.',
  'datePlan.viewSummary': 'See the summary →',
  'planEdit.title': 'Edit the route',
  'planEdit.body': 'Reorder or drop stops (up to {{max}}). GoGo recalculates times, travel and cost.',
  'planEdit.locked': 'locked',
  'planEdit.moveUp': 'Move up',
  'planEdit.moveDown': 'Move down',
  'planEdit.saveFailed': 'Could not save. Please try again.',
  'planEdit.conflict': 'The plan changed elsewhere. Reload and edit again.',
  'planEdit.hostOnlyBody': 'Only the host can edit the route.',
  'datePlan.edit': 'Edit the route',
  'datePlan.superseded': 'A newer plan replaced this one — tap to open the room.',

  'placeDetail.goodFor': 'Good for',
  'placeDetail.coupleRatings': 'Couple ratings',
  'placeDetail.addToPlan': 'Add to plan',
  'placeDetail.openNow': 'Open now · Closes 23:00',
  'placeDetail.priceCouple': '~450k · Total for 2',
  'placeDetail.rating': '8.9 · 126 ratings from the GoGo community',
  'placeDetail.updated': 'Last check unknown',
  'placeDetail.updatedAt': 'Last checked {{date}}',
  'placeDetail.ratingCount': '{{count}} ratings',
  'placeDetail.priceUnit.per_person': 'per person',
  'placeDetail.priceUnit.per_item': 'per item',
  'placeDetail.priceUnit.per_hour': 'per hour',
  'placeDetail.priceUnit.per_night': 'per night',
  'placeDetail.report': 'Report incorrect information',
  'placeDetail.reportUnavailable': 'Reporting is not available yet.',
  'placeDetail.ratingsTitle': 'Ratings',
  'placeDetail.noRating': 'No rating yet',
  'placeDetail.avgVisit': 'typical visit',
  'placeDetail.attributes': 'Facilities',
  'placeDetail.showHours': 'See opening hours',
  'placeDetail.hideHours': 'Hide',
  'placeReviews.title': 'Latest GoGo reviews',
  'placeReviews.source': 'From the GoGo community, moderated. Never combined with the Google rating.',
  'placeReviews.empty': 'No approved GoGo reviews for this place yet.',
  'placeReviews.error': "Couldn't load GoGo reviews.",
  'placeReviews.offline': "You're offline, so GoGo reviews couldn't load.",
  'placeReviews.loading': 'Loading GoGo reviews',
  'placeReviews.deletedAuthor': 'Deleted user',
  'placeReviews.rating': '★ {{rating}}/5',
  'placeReviews.ratingA11y': '{{rating}} out of 5 stars',
  'placeReviews.writtenOn': 'Written {{date}}',
  'placeReviews.orderLatest': 'Latest',
  'placeReviews.orderHelpful': 'Most helpful',
  'placeReviews.helpfulFallback': 'No review has been marked helpful yet, so the latest are shown.',
  'placeReviews.helpful': 'Helpful · {{count}}',
  'placeReviews.helpfulA11y': '{{count}} found this helpful',
  'placeReviews.markHelpful': 'Mark as helpful',
  'placeReviews.unmarkHelpful': 'Remove helpful mark',
  'placeReviews.signInToReact': 'Sign in to mark reviews as helpful.',
  'placeReviews.helpfulOwn': "You can't mark your own review.",
  'placeReviews.helpfulFailed': "Couldn't save that. Please try again.",
  'common.weekday.0': 'Sunday',
  'common.weekday.1': 'Monday',
  'common.weekday.2': 'Tuesday',
  'common.weekday.3': 'Wednesday',
  'common.weekday.4': 'Thursday',
  'common.weekday.5': 'Friday',
  'common.weekday.6': 'Saturday',
  'placeDetail.save': 'Save',
  'placeDetail.createRoom': '💌 Start a hangout from this place',

  'activeDate.live': 'Date in progress',
  'activeDate.stop': 'Stop {{n}} / {{total}}',
  'activeDate.ongoing': 'Happening now',
  'activeDate.next': 'Up next',
  'activeDate.doneStep': 'Done here ✓',
  'activeDate.finish': 'Finish date 🎉',
  'activeDate.completeFailed': 'Could not save. Check your connection and try again.',
  'activeDate.completeFailedRetry': 'Could not save. Please try again.',
  'activeDate.notActive': "The host starts it from the plan screen. Until then, stops can't be marked done or checked in.",
  'activeDate.backToPlan': 'Back to the plan',
  'activeDate.notActiveTitle': "The outing hasn't started",
  'activeDate.endedTitle': 'This outing is over',
  'activeDate.endedBody': 'Look back at the stops and leave a review.',

  'checkin.title': 'Check in here',
  'checkin.rate': 'How was this stop?',
  'checkin.addPhoto': '📷 Add check-in photos',
  'checkin.photoCount': '{{n}}/3 photos',
  'checkin.notePlaceholder': 'Quick thoughts… (optional)',
  'checkin.billToggle': '💳 Check in the bill split by people',
  'checkin.billHint': 'Helps the community see real prices. Bill photo required for verification.',
  'checkin.billTotal': 'Bill total (k VND)',
  'checkin.billPerPerson': '≈ {{amount}}/person · {{n}} people',
  'checkin.billPeople': 'People',
  'checkin.billPeopleHint': 'Actual headcount at this stop — not always the whole group.',
  'checkin.billPhoto': '📸 Add bill photo (required)',
  'checkin.billPhotoDone': '✓ Bill photo added',
  'checkin.billRequired': 'Bill total and photo are required to save.',
  'checkin.save': 'Save check-in ✓',
  'checkin.skip': 'Skip',
  'checkin.saving': 'Saving…',
  'checkin.saveFailed': 'Could not save the check-in. Try again, or skip to move on.',
  'checkin.saveFailedOffline': 'Could not save the check-in. Check your connection and try again.',
  'checkin.saveRefused': 'This check-in cannot be saved. Skip to move on.',
  'checkin.photoFailed': 'An image did not upload. The check-in still saves, just without it.',
  'checkin.removePhoto': 'Remove this photo',
  'checkin.tag.would_return': 'Would go again',
  'checkin.tag.easy_conversation': 'Easy conversation',
  'checkin.tag.photogenic': 'Photogenic',
  'checkin.tag.worth_the_money': 'Worth the money',
  'checkin.tag.just_okay': 'Just okay',
  'checkin.tag.not_a_fit': 'Not a fit',
  'activeDate.saving': 'Saving…',
  'activeDate.allDoneTitle': 'Every stop is done',
  'activeDate.allDoneBody': 'This plan is complete.',
  'dateFinished.checkinTitle': 'Your check-ins',

  'dateFinished.title': 'Date complete!',
  'dateFinished.body': 'Hope you all had a great evening.',
  'dateFinished.body_couple': 'Hope you two had a great evening.',
  'dateFinished.cta': 'How was it? 💬',
  'dateFinished.cta_couple': 'How was it, you two? 💬',
  'dateFinished.stopsDone': '{{done}}/{{total}} stops done',

  'review.title': 'How was\nthis date?',
  'review.body': 'Your rating helps GoGo pick better next time.',
  'review.chooseStars': 'Pick a star rating',
  'review.rating.bad': 'Oh no 😢',
  'review.rating.ok': 'Not bad 😊',
  'review.rating.great': 'Great! 😍',
  'review.rating.perfect': 'Perfect! 🤩',
  'review.highlights': 'Highlights',
  'review.placeholder': 'Anything GoGo should know? (optional)',
  'review.submit': 'Submit review',
  'review.starAria': '{{n}} stars',
  'review.submitting': 'Sending…',
  'review.successTitle': 'Review sent',
  'review.successPending': 'Your review is being checked. It appears on the place page once it is approved.',
  'review.successPublished': 'Thank you! Your review is live on the place page.',
  'review.continue': 'Continue',
  'review.skip': 'Later',
  'review.failed': 'Could not send your review. Please try again.',
  'review.moderationNote': 'Reviews are checked before they appear publicly.',

  'myReviews.title': 'My reviews',
  'myReviews.signInBody': 'Reviews belong to your account.',
  'myReviews.emptyTitle': 'No reviews yet',
  'myReviews.emptyBody': 'After a date you can rate the places you went.',
  'myReviews.edit': 'Edit review',
  'myReviews.editResetsModeration': 'Editing sends it back for review before it appears.',
  'myReviews.status.pending': 'Pending review',
  'myReviews.status.published': 'Published',
  'myReviews.status.rejected': 'Rejected',
  'myReviews.status.removed': 'Removed',

  'sharedResult.title': 'Our Date ✨',
  'sharedResult.common': 'What you share',
  'sharedResult.common_couple': 'What you two share',
  'sharedResult.stats': 'Stats',
  'sharedResult.nextDate': 'Plan the next date 🚀',
  'sharedResult.share': 'Share story',
  'sharedResult.matchScore': 'GoGo match score',
  'sharedResult.stops': 'Stops',
  'sharedResult.perPerson': 'Per person',
  'suggestion.component.budget': 'Fits the budget',
  'suggestion.component.quality': 'Quality',
  'suggestion.component.distance': 'Distance',
  'suggestion.component.consensus': 'Everyone agreed',
  'suggestion.component.freshness': 'Fresh data',
  'suggestion.component.preference': 'Matches preferences',
  'suggestion.component.suitability': 'Suits the occasion',
  'suggestion.component.seedBoost': 'You suggested it',

  'search.title': 'Find places',
  'search.placeholder': 'Search venues, activities, areas…',
  'search.openUntil': 'Open · Closes {{time}}',
  'search.closedOpens': 'Closed · Opens {{time}}',
  'search.filters': 'Filters',
  'search.filterDistance': 'Max distance (km)',
  'search.filterPrice': 'Price range (k VND / person)',
  'search.priceFrom': 'From',
  'search.priceTo': 'To',
  'search.distancePlaceholder': 'e.g. 5',
  'search.filterSuited': 'Suited for',
  'search.filterCategory': 'Category',
  'search.filterStyle': 'Mood & setting',
  'search.apply': 'Apply',
  'search.clearFilters': 'Clear filters',
  'search.empty': 'No places found',
  'search.emptyHint': 'Try another keyword or add a new place from Google Maps.',
  'search.distanceNeedsArea': 'Pick a starting area first to filter by distance.',
  'search.reason.TEXT_MATCH': 'Matches your search',
  'search.reason.NEAR_YOU': 'Near you',
  'search.reason.HIGHLY_RATED': 'Highly rated',
  'search.reason.CURATED': 'Curated by GoGo',
  'search.reason.OPEN_NOW': 'Open now',
  'search.addNew': 'Add a place from Google Maps',

  'placeImport.title': 'Add a new place',
  'placeImport.body': 'Paste a Google Maps link — GoGo verifies the place is real and trustworthy before adding it.',
  'placeImport.placeholder': 'https://maps.app.goo.gl/…',
  'placeImport.verify': 'Verify place',
  'placeImport.verifying': 'Verifying on Google Maps…',
  'placeImport.invalidUrl': 'That does not look like a Google Maps link.',
  'placeImport.verifiedBadge': '✓ Verified on Google Maps',
  'placeImport.reviews': '★ {{rating}} · {{n}} Google reviews',
  'placeImport.addToSaved': 'Add to Saved',
  'placeImport.added': '✓ Added to Saved',
  'placeImport.rejectedTitle': 'This place cannot be added',
  'placeImport.reason.NOT_FOUND': 'Place not found on Google Maps.',
  'placeImport.reason.INSUFFICIENT_REVIEWS': 'Not enough trusted reviews yet (minimum 10).',
  'placeImport.reason.LOW_RATING': 'Rating is below the GoGo quality bar.',
  'placeImport.reason.OUT_OF_AREA': 'Outside the area GoGo currently serves.',
  'placeImport.tryAgain': 'Try another link',
  'placeImport.submit': 'Send this place to GoGo',
  'placeImport.submitFailed': 'Could not send it. Please try again.',
  'placeImport.pendingReview': '✓ Sent — GoGo reviews it before it appears.',
  'placeImport.existsTitle': 'Already on GoGo',
  'placeImport.existsBody': 'This place is already in the catalog.',
  'placeImport.openPlace': 'Open place',
  'placeImport.pickCandidate': 'The link matched several places — pick the right one:',
  'placeImport.rateLimited': 'That is a lot of tries. Wait a moment and try again.',
  'placeImport.providerUnavailableTitle': 'Cannot verify right now',
  'placeImport.providerUnavailable':
    'GoGo cannot verify places at the moment. Please try again later.',
  'placeImport.businessStatus.CLOSED_TEMPORARILY': '⚠️ Google reports this place as temporarily closed.',
  'placeImport.businessStatus.CLOSED_PERMANENTLY': '⚠️ Google reports this place as permanently closed.',
  'placeImport.businessStatusUnknown': '⚠️ Google has not confirmed this place is operating.',
  'placeImport.reason.CLOSED': 'This place has closed.',
  'placeImport.reason.INVALID_URL': 'That link is invalid or broken.',
  'placeImport.reason.PROVIDER_ERROR': 'Could not reach Google Maps. Try again later.',
  'placeImport.metaTitle': 'Extra details (optional)',
  'placeImport.metaCategory': 'Category',
  'placeImport.metaVibes': 'Vibes (up to {{max}})',
  'placeImport.metaPrice': 'Estimated price per person (thousands)',
  'placeImport.metaNote': 'Note for the moderator…',
  'placeImport.submissionStatus.pending': 'Waiting for review',
  'placeImport.submissionStatus.approved': '✓ Approved',
  'placeImport.submissionStatus.rejected': 'Rejected',
  'placeImport.submissionStatus.merged': 'Merged into an existing place',
  'saved.addPlace': 'Add place',
  'saved.newTag': 'Just added',

  'plans.emptyHistory': 'No plans in your history yet.',
  'plans.overdue': 'Past its date',
  'roomSchedule.title': 'Plan schedule',
  'roomSchedule.format': 'Enter day/month/year and hour:minute in your device time zone.',
  'roomSchedule.start': 'Start',
  'roomSchedule.end': 'End (optional)',
  'roomSchedule.placeholder': '10/09/2026 19:00',
  'roomSchedule.invalid': 'Enter a valid date and time; the end must follow the start.',
  'roomSchedule.locked': 'This plan can no longer be rescheduled.',
  'roomSchedule.unset': 'Not scheduled',
  'plans.title': 'Plans',
  'plans.upcoming': 'Upcoming',
  'plans.past': 'History',
  'plans.loadMore': 'Load more',
  'plans.untitled': 'Untitled plan',
  'plans.emptyTitle': 'No plans yet',
  'plans.emptyBody': 'Create one, or open an invite link to get started.',
  'plans.emptySignedOut': 'Sign in and create one, or open an invite link you were sent.',
  'plans.status.draft': 'Draft',
  'plans.status.collecting': 'Waiting on picks',
  'plans.status.matching': 'Matching',
  'plans.status.ready': 'Plan ready',
  'plans.status.active': 'In progress',
  'plans.status.completed': 'Done',
  'plans.status.cancelled': 'Cancelled',
  'plans.status.expired': 'Expired',

  'saved.title': 'Saved',
  'saved.filter.all': 'All',
  'saved.filter.places': 'Places',
  'saved.filter.plans': 'Plans',
  'saved.remove': 'Remove from saved',
  'saved.currentArea': 'Your area',
  'saved.group.multipleProvinces': 'Several provinces',
  'saved.group.multipleCommunes': 'Several wards and communes',
  'saved.group.unknown': 'Area not known yet',
  'saved.plan.title': 'Saved plan',
  'saved.plan.stops': '{{n}} stops',
  'saved.plan.stale': 'Conditions changed after this plan was made',
  'saved.plan.unavailable': 'A stop is no longer available',
  'saved.plan.missing': 'This plan cannot be opened',
  'saved.placeUnavailable': 'This place is no longer available',
  'saved.emptyTitle': 'Nothing saved yet',
  'saved.emptyBody': 'Save places while searching and they show up here.',
  'saved.browse': 'Find places',
  'saved.signInTitle': 'Sign in to save places',
  'saved.signInBody': 'Saved items belong to your account.',
  'saved.mapUnavailable': "The map can't open on this device.",

  'profile.title': 'Your profile',
  'profile.logout': 'Log out',
  'profile.loggingOut': 'Logging out…',
  'profile.logoutFailed': 'Could not sign out. Check your connection and try again.',
  'profile.guestName': 'Guest',
  'profile.guestSubtitle': 'No account yet',
  'profile.shortcut.saved': 'Saved',
  'profile.shortcut.plans': 'Plans',
  'profile.shortcut.reviews': 'Reviews',

  'notifications.title': 'Notifications',
  'notifications.emptyTitle': 'Nothing yet',
  'notifications.emptyBody': 'When someone joins a room or a plan is ready, it shows up here.',
  'notifications.signInTitle': 'Sign in to see notifications',
  'notifications.signInBody': 'Notifications belong to your account.',
  'notifications.kind.invite': 'Room invite',
  'notifications.kind.preference_reminder': 'Reminder to pick',
  'notifications.openRefused': 'This notification can’t be opened. It may have been removed, or you no longer have access.',
  'notifications.openRetry': "Couldn't open this notification. Check your connection and tap it again.",
  'notifications.kind.plan_ready': 'Plan is ready',
  'notifications.kind.plan_changed': 'Plan changed',
  'notifications.kind.date_reminder': 'Date reminder',
  'notifications.kind.moderation_update': 'Moderation update',
  'notifications.pushUnavailable': 'This notification can’t be opened. It may have been removed, or you no longer have access.',
  'permissionsSettings.title': 'Notifications and location',
  'permissionsSettings.notifications': 'Notifications',
  'permissionsSettings.location': 'Location',
  'permissionsSettings.areaInAccount': 'Choose your default area in Account',
  'notificationSettings.saveFailed': 'Could not save. The switch is back to its previous state.',
  'notificationSettings.retrySave': 'Try saving again',
  'notificationSettings.loadFailed': 'Could not load this account\'s notification setting.',
  'notificationSettings.switchLabel': 'Notifications',
  'notificationSettings.switchHint': 'Applies to every push notification for this account, on every device.',
  'notificationSettings.inboxNote': 'Your in-app inbox keeps every notification, even when this is off.',
  'notificationSettings.migratedOff': 'Off because you previously turned off at least one kind of notification. Turn it on to receive them all.',
  'notificationSettings.deviceTitle': 'On this device',
  'notificationSettings.pushNote': 'Notifications for GoGo are off in this device\'s Settings. Even with the switch above on, this device will not show notifications until you allow them.',
  'notificationSettings.openSettings': 'Open notification settings',
  'notificationSettings.openSettingsFailed': 'Could not open Settings. Open system Settings and select GoGo.',
  'notificationSettings.pushChecking': 'Checking notification permission on this device…',
  'notificationSettings.pushReady': 'This device allows GoGo to show notifications.',
  'notificationSettings.pushUnknown': 'Could not check notification permission on this device.',
  'profile.settings.notificationsLocation': '🔔 Notifications and location',
  'profile.settings.inbox': '📬 Inbox',
  'profile.settings.reviews': '⭐ My reviews',
  'profile.settings.account': '⚙️ Account',
  'profile.settings.comingSoon': 'Not built yet',
  'locationSettings.statusTitle': 'Location permission on this device',
  'locationSettings.status.checking': 'Checking…',
  'locationSettings.status.granted': 'Allowed.',
  'locationSettings.status.undetermined': 'Not asked yet. GoGo asks only when you need it.',
  'locationSettings.status.denied': 'Turned off in Settings. You can still pick an area by hand.',
  'locationSettings.status.unavailable': 'This device cannot provide a location. Pick an area by hand.',
  'locationSettings.body': 'GoGo uses your location only when you tap "Current location" while creating a date, to find places near you. Your profile keeps a home area you choose, never a position.',
  'locationSettings.openSettings': 'Open Settings',
  'locationSettings.allow': 'Allow',
  'locationSettings.note': "A date's exact starting point is kept only while the date is live, then cleared under the retention policy.",
}

export const enContent: LocaleContent = {
  budgetOptions: [
    { label: 'Under 300k', sub: 'Coffee · a walk' },
    { label: '300–500k', sub: 'Light bites · bubble tea' },
    { label: '500–800k', sub: 'Dinner · a movie' },
    { label: '800k–1.5m', sub: 'Restaurant · workshop' },
    { label: "Doesn't matter", sub: 'As long as it’s fun' },
  ],
  moods: [
    { emoji: '❤️', label: 'Romantic' },
    { emoji: '😌', label: 'Chill' },
    { emoji: '🎨', label: 'Creative' },
    { emoji: '🎮', label: 'Playful' },
    { emoji: '🌃', label: 'Night vibe' },
  ],
  settings: [
    { emoji: '🏠', label: 'Indoor' },
    { emoji: '🌿', label: 'Outdoor' },
    { emoji: '🤫', label: 'Quiet' },
    { emoji: '🎶', label: 'Lively' },
  ],
  spendingStyles: [
    { emoji: '💸', label: 'Budget' },
    { emoji: '⚖️', label: 'Balanced' },
    { emoji: '✨', label: 'Fancy' },
  ],
  quickPresets: [
    { emoji: '🌙', label: 'Tonight' },
    { emoji: '☀️', label: 'Weekend' },
    { emoji: '🎉', label: 'Special occasion' },
  ],
  refinementOptions: [
    { emoji: '💸', label: 'Cheaper' },
    { emoji: '📍', label: 'Closer' },
    { emoji: '🛵', label: 'Less travel' },
    { emoji: '🍜', label: 'Different food' },
    { emoji: '🎨', label: 'Different activity' },
    { emoji: '🔄', label: 'Completely different' },
  ],
  groupMatchReasons: [
    'Highest voted option',
    'Within the per-person budget',
    'Everything is still open',
    'About 12 minutes of travel total',
  ],
  tagLabels: {} as Record<string, string>,
  prefOptions: [
    { emoji: '🍜', label: 'Good food' },
    { emoji: '🎭', label: 'Do something fun' },
    { emoji: '💬', label: 'Sit and talk' },
    { emoji: '🚶', label: 'Take a walk' },
    { emoji: '🌟', label: 'Try something new' },
    { emoji: '🧋', label: 'Grab a drink' },
    { emoji: '📸', label: 'Take photos' },
    { emoji: '😴', label: 'Just chill' },
  ],
  reviewTags: [
    { emoji: '❤️', label: 'Would go again' },
    { emoji: '💬', label: 'Easy conversation' },
    { emoji: '📸', label: 'Photogenic' },
    { emoji: '💸', label: 'Worth the money' },
    { emoji: '😐', label: 'Just okay' },
    { emoji: '👎', label: 'Not a fit' },
  ],
  savedFilters: ['All', 'Food', 'Activities', 'Plans'],
  searchFilters: ['All', 'Food', 'Coffee', 'Activities', 'Nightlife', 'Stay'],
  suitedOptions: ['Couples', 'Groups'],
  stopCategories: [
    { emoji: '🍣', label: 'Dinner' },
    { emoji: '🎨', label: 'Activity' },
    { emoji: '🍰', label: 'Dessert' },
  ],
  matchReasons: [
    'You all like Japanese',
    'You all picked Creative',
    'Within the 500–800k budget',
    'Everything is still open',
    'Nothing over 5 km away',
  ],
  matchingMessages: ['Finding what you all like…', 'Assembling the itinerary…', 'Matched 🎉'],
  dateSummaryChips: ['🌙 Tonight', '📍 Thao Dien', '⏱ 2–3 hrs', '💰 500–800k', '❤️ Romantic', '🎨 Creative'],
  dateDetails: [
    ['🌙', 'Tonight'],
    ['📍', 'Thao Dien, ≤ 5 km'],
    ['⏱', '2–3 hrs · Starts ~18:30'],
    ['💰', '~800k for 2 people'],
    ['💝', 'Romantic · Creative'],
  ],
  settingsItems: ['🔔 Notifications', '📍 Location', '🔒 Privacy', '⚙️ Account'],
}
