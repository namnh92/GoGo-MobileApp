...
| --- | --- | --- |
| Theme, tokens, variants | `react-native-unistyles` 3.3.0 + `react-native-nitro-modules` 0.36.1 (pinned) | Chốt — installed (ADR-0009): four accent themes in `src/shared/ui/theme.ts`, config in `src/shared/ui/unistyles.ts` (loaded from `index.ts`); migrate `.style.tsx` incrementally, keep the `.view/.style` split |
