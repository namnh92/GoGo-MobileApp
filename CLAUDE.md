...
- **Flavours:** `EXPO_PUBLIC_ENV` (`development` | `staging` | `production`) drives bundle id `max.gogo.{flavor}`, app name and scheme from `app.config.ts` — the single source, since `ios/`/`android/` are prebuild output. An unknown value fails the build rather than defaulting.
- Deep links: `https://gogo.app/r/{inviteCode}` (**production only** — dev/staging cannot verify against a domain whose assetlinks never names them), `gogo://room/{inviteCode}`, `gogo://plans/{planId}`, `gogo://places/{placeId}`; dev/staging use `gogo-dev://` / `gogo-staging://`. Handle cold/warm start × logged-in/guest × expired invite × app-not-installed. Invite codes carry no PII.
